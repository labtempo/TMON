...
implementation {
	...
	uint8_t firsts_battery_msg = 0; //Initial battery msgs sent
	
	uint16_t packetsToSerial = 0;// Variables simulation
	uint16_t packetsToNetwork = 0;// Variables simulation	

	/* Current local state */
	oscilloscope_t local_temp_msg;
	...
	event void Boot.booted() {		
		local_temp_msg.id = TOS_NODE_ID;
		local_temp_msg.type = OSCILLOSCOPE_MSG_TYPE;
		local_temp_msg.count = 0;
	...
	event void RadioControl.startDone(error_t error) {
		...
		if(sizeof(local_temp_msg) > call Send.maxPayloadLength()) 
			fatal_problem(7);

		if(call SerialControl.start() != SUCCESS) 
			fatal_problem(5);
		dbg("Ctrl", "RadioControl start done.\n");
	}
	...
	event void SerialControl.startDone(error_t error) {
		...
		if(local_temp_msg.id == BASE_STATION_ID) {
			call RootControl.setRoot();
			dbg("Ctrl", "I'm now root.\n");
		}
		else
		{
			if (local_temp_msg.id % ROUTER_NODE_DIVIDER != 0){
				/*
			...
			startBatteryTimer(1024);
		}
		dbg("Ctrl", "SerialControl done.\n");
	}
	...
	task void uartSendTask() {
		...
		out = (oscilloscope_t * ) call SerialSend.getPayload(&uartbuf,
				sizeof(oscilloscope_t));
		dbg("Ctrl", "Uart send task: id=%d, counter=%d\n", out->id, out->count);
	...
	event void TempTimer.fired() {		
		...
		if (last_reading != NULL_TEMP_READING && local_temp_msg.reading != NULL_TEMP_READING)
		{					
			t_diff = temp_diff(last_reading, local_temp_msg.reading);			
			dbg("Temp", "Everything is ok, temp diff is: %f\n", t_diff);	
		}
		else if (local_temp_msg.reading == NULL_TEMP_READING) // assure that will not send if the current_temp is messed up
		{
			suppressed_temp_msgs = suppressed_temp_msgs == MAX_TEMP_SUPPRESSION_MSGS ? suppressed_temp_msgs - 1: suppressed_temp_msgs;
			//dbg("Temp", "Curr temp is messed up: %d\n", local_temp_msg.reading);
		}
		...
		if(t_diff >= TEMPERATURE_DELTA || suppressed_temp_msgs == MAX_TEMP_SUPPRESSION_MSGS) {			
			dbg("Temp", "Sending temp reading: %d. Temp diff is: %f\n", local_temp_msg.reading, t_diff);

			atomic {
				disseminate(local_temp_msg);
			}
			suppressed_temp_msgs = 0;
			last_reading = local_temp_msg.reading;
			if (local_temp_msg.count == MAX_COUNT)
				local_temp_msg.count = 0;
			else
				local_temp_msg.count++;
		}
		else
		{
	...
	event void BatteryTimer.fired() {		
		if (call Voltage.read() != SUCCESS) 
			fatal_problem(7);
		dbg("Battery", "Sending battery reading: %d\n", local_battery_msg.reading);
		...
		if (firsts_battery_msg <= MAX_INITIAL_BATTERY_MSGS){
			firsts_battery_msg++;
			if (firsts_battery_msg==MAX_INITIAL_BATTERY_MSGS)
	...
	event void LightTimer.fired() {
		...
		{
			dbg("Light", "Sending light reading: %d\n", local_light_msg.reading);
	...
	event void Temp.readDone(error_t result, uint16_t data) {
		...
		local_temp_msg.reading = (data < MAX_TEMP_READING && data > MIN_TEMP_READING) ? data : NULL_TEMP_READING;
