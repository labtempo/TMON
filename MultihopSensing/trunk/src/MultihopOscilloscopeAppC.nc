...
implementation {
	components MainC, MultihopOscilloscopeC, LedsC, new TimerMilliC() as TimerMilliTemp, 
	new TimerMilliC() as TimerMilliBattery, new TimerMilliC() as TimerMilliLight,
	new TempC() as TempSensor;
	
	#ifndef TOSSIM
		components DelugeC, new VoltageC() as VoltageSensor;
		DelugeC.Leds->LedsC;		
		components new PhotoC() as LightSensor;
	#endif
	
	#ifdef TOSSIM
		components new ConstantSensorC(uint16_t, 3000) as VoltageSensor;
		components new ConstantSensorC(uint16_t, 2000) as LightSensor;
	#endif
	
	MultihopOscilloscopeC.Boot->MainC;
	MultihopOscilloscopeC.TempTimer->TimerMilliTemp;
	MultihopOscilloscopeC.BatteryTimer->TimerMilliBattery;
	MultihopOscilloscopeC.LightTimer->TimerMilliLight;		
	MultihopOscilloscopeC.Temp->TempSensor;
	MultihopOscilloscopeC.Voltage->VoltageSensor;
	MultihopOscilloscopeC.Light->LightSensor;
	...
	components new PoolC(message_t, 10) as UARTMessagePoolP, new QueueC(message_t 
			*, 10) as UARTQueueP;

	MultihopOscilloscopeC.UARTMessagePool->UARTMessagePoolP;
	MultihopOscilloscopeC.UARTQueue->UARTQueueP;

	components new PoolC(message_t, 20) as DebugMessagePool, new QueueC(message_t 
			*, 20) as DebugSendQueue,
