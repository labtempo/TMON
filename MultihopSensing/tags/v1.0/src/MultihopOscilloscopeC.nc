...
//		dbg("Radio",
//				"Sample [interval = %d, version = %d, count = %d] received from %d.\n", in->interval, 
//				in->version, in->count, in->id);
	...
	event message_t * Snoop.receive(message_t * msg, void * payload,
		...
		if(omsg->version > local.version) {
			dbg("Radio", "Updating interval.\n");
			local.version = omsg->version;
			local.interval = omsg->interval; //sampling period
			startTimer();
		}
	...
	event void Timer.fired() {
		...
		if(reading == NREADINGS) {
			dbg("Temp", "Sample buffer is full.\n");
