...
implementation {
	...
	static float tempDifference(uint16_t adc_a, uint16_t adc_b);
	void sendMessage();
	void failedSend();
	...
	uint8_t initialSendPhase = 0;
	uint16_t suppressionMsgs = INITIAL_MAX_SUPPRESSION_MSGS;
...
//		uint8_t is_synced = call GlobalTime.getGlobalTime(&global_timestamp);
//		uint32_t local_timestamp = call GlobalTime.getLocalTime();
//		double skew = (double)call TimeSyncInfo.getSkew();
//        uint16_t ftsp_root_addr = call TimeSyncInfo.getRootID();
//        uint16_t ftsp_table_entries = call TimeSyncInfo.getNumEntries();
//        printf("global_timestamp %ld | local_timestamp %ld | is_synced %d | skew %lf | ftsp_root_addr %d | ftsp_table_entries %d \n", global_timestamp, local_timestamp, is_synced, skew, ftsp_root_addr, ftsp_table_entries);
//        printfflush();
		...
		{
			suppressedTemperatures = suppressedTemperatures == suppressionMsgs ? suppressedTemperatures - 1: suppressedTemperatures;
		}else // (last_reading <= 0) make sure that will send regardless of tempDifference 
		{
			suppressedTemperatures = suppressionMsgs;
		...
		if((tempDiff > TEMPERATURE_DELTA) | (suppressedTemperatures == suppressionMsgs) | (lightDiff > LIGHT_DIFF_THRESHOLD) | (lastEtx != etxNow) ) {			
			dbg("MAster", "Sending a reading: %d. Temp diff is: %f\n", temperatureNow, tempDiff);
			atomic{
				if (!sendBusy)
					sendMessage();
					initialSendPhase++;
					if (initialSendPhase == NUMBER_INITIAL_MSGS){
						suppressionMsgs = MAX_SUPPRESSION_MSGS;																	
						}						
			}
	...
	void sendMessage() {
		...
		TMON_t*	msg = (TMON_t*)call Send.getPayload(&packet, sizeof(TMON_t));
		...
		msg->parent = parent;
		msg->metric = etxNow;
		is_synced = call GlobalTime.getGlobalTime(&delay);
		msg->delay = delay;
		
        if (call Send.send(&packet, sizeof(TMON_t)) != SUCCESS) {
			failedSend();
			dbg("TMONC", "%s: Transmission failed.\n", __FUNCTION__);
		}
		else {
			sendBusy = TRUE;
			seqno++;
			dbg("TMONC", "%s: Transmission succeeded.\n", __FUNCTION__);
			//call Leds.led2Toggle();	// To view when is sending 	
		}
	}
	...
	Receive.receive(message_t* msg, void* payload, uint8_t len) {
		...
		if (!call Pool.empty() && call Queue.size() < call Queue.maxSize()) {
			...
			TMON_t*	msgChange = (TMON_t*)payload;
			is_synced = call GlobalTime.getGlobalTime(&newTime);
			msgChange->delay = (int)((newTime - msgChange->delay)*0.9765625); // Normalization to milliseconds
